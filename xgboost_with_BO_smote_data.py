...
print("--- SCRIPT: BO-SMOTE (BORDERLINE-SMOTE) + TUNED XGBOOST ---")
...
print("3. Tuning BO-SMOTE + XGBoost with GridSearch...")
pipeline = Pipeline([
...
plt.title('Real-World Test Data - Confusion Matrix (BO-SMOTE + XGBoost)')
...
bosmote_export_df.to_csv('BO_SMOTE_Combined_Training_Data_XGB.csv', index=False)
print(f"Saved -> 'BO_SMOTE_Combined_Training_Data_XGB.csv'")
...
sns.heatmap(cm_bosmote, annot=True, fmt='d', cmap='Greens', 
            xticklabels=["Predicted Safe", "Predicted Hotspot"], 
            yticklabels=["Actually Safe", "Actually Hotspot"])
plt.title('Training Data (with BO-SMOTE) - Confusion Matrix (XGBoost)')
plt.ylabel('BO-SMOTE Reality (Real + Fake)')
plt.xlabel('Algorithm Prediction')
plt.tight_layout()
plt.savefig('Matrix_BO_SMOTE_Training_Data_XGB.png', dpi=300)
print("Saved -> 'Matrix_BO_SMOTE_Training_Data_XGB.png'")
